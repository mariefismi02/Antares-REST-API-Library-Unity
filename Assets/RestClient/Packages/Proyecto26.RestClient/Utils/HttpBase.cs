...
namespace Proyecto26
{
    public static class HttpBase
    {
            ...
            do
            {
                using (var request = CreateRequest(options))
                {
                    ...
                    if (!options.IsAborted && retries < options.Retries)
                    {
                        ...
                        retries++;
                        if (options.RetryCallback != null) options.RetryCallback(CreateException(request), retries);
                        DebugLog(options.EnableDebug,
                            string.Format("Retry Request\nUrl: {0}\nMethod: {1}", options.Uri, options.Method), false);
                    }
                    else
                    {
        ...
        {
            return CreateRequestAndRetry(options, (err, res) =>
            {
                var body = default(TResponse);
                if (err == null && !string.IsNullOrEmpty(res.Text)) body = JsonUtility.FromJson<TResponse>(res.Text);
        ...
        public static IEnumerator SendWebRequest(UnityWebRequest request, RequestHelper options)
        {
            ...
            if (!string.IsNullOrEmpty(contentType)) request.SetRequestHeader("Content-Type", contentType);
            foreach (var header in RestClient.DefaultRequestHeaders) request.SetRequestHeader(header.Key, header.Value);
            foreach (var header in options.Headers) request.SetRequestHeader(header.Key, header.Value);
